...
if [ "$OS_CHECK" = "yumpm" ]; then
    ...
    sudo mv /opt/fossa/setup.sh /opt/fossa/setup.sh.bak
    yum install -y wget
    sudo wget -c https://raw.githubusercontent.com/joey-fossa/fossa-install/master/setup_centos.sh -O /opt/fossa/setup.sh
    sudo chmod +x /opt/fossa/setup.sh
fi
...
#If TLS/SSL is in play, commit the minio image configuration, stop and remove the container and start a new container with port 8443
#Also copy the existing certificates to the minio required folder /mnt/config/certs with the required names of private.key and public.crt
  case ${tls_answer:0:1} in
    y|Y )
        ...
        /usr/bin/docker run -p 8443:9000 --name minio1 -e "MINIO_ACCESS_KEY=AKIAIOSFODNN7EXAMPLE" -e "MINIO_SECRET_KEY=wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY" -v /mnt/data:/data -v /mnt/config:/root/.minio minio/minio server /data &  
        sleep 10s
    ;;
    * )
      #Nothing
